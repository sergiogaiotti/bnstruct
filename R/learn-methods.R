...
setMethod(
  ...
  function(x, y = NULL, algo = "mmhc", scoring.func = "BDeu", initial.network = NULL,
    ...
    if (is.null(y) || !inherits(y, "BNDataset")) {
      stop(
        "A BNDataset must be provided in order to learn a network from it. ",
    ...
    if (num.time.steps(dataset) > 1) {
      bn <- learn.dynamic.network(
        bn, dataset, num.time.steps(dataset), algo, scoring.func,
      ...
      bn <- learn.structure(
        bn, dataset, algo, scoring.func, initial.network, alpha, ess,
  ...
  function(bn, dataset, algo = "mmhc", scoring.func = "BDeu", initial.network = NULL,
    ...
    if (algo == "sm") {
      ...
      if (max.fanin < max.parents ||
        ...
        bnstruct.log(
          "SM uses 'max.parents' and 'max.parents.layers' parameters, ",
      ...
      if (bootstrap) {
        ...
        for (i in seq_len(num.boots(dataset)))
        {
          ...
          dag <- sm(
            data, node.sizes, scoring.func, cont.nodes, max.parents, layering,
        ...
        dag(bn) <- sm(
          data, node.sizes, scoring.func, cont.nodes,
    ...
    if (algo == "hc") {
      if (max.parents < max.fanin ||
        ...
        bnstruct.log(
          "HC uses 'max.fanin' and 'layer.struct' parameters, ",
    ...
    if (algo == "hc_disc") {
      ...
      if (bootstrap) {
        ...
        for (i in seq_len(num.boots(dataset)))
        {
          data <- boot(dataset, i, use.imputed.data = use.imputed.data)
          cpc <- matrix(rep(1, num.nodes * num.nodes), nrow = num.nodes, ncol = num.nodes)
          out.hc <- hc_w_disc(data, node.sizes, scoring.func, cpc, cont.nodes,
            ess = ess,
            tabu.tenure = tabu.tenure, max.parents = max.parents, init.net = in.dag,
            wm.max = wm.max, layering = layering, layer.struct = layer.struct,
        ...
        node.sizes(bn)[cont.nodes] <- sapply(discr[cont.nodes], function(x) length(x) + 1L)
