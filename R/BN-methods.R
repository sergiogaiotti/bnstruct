...
#' @aliases name,BN
#' @rdname name
setMethod("name", "BN", function(x) {
  return(slot(x, "name"))
})

#' @aliases num.nodes,BN
#' @rdname num.nodes
setMethod("num.nodes", "BN", function(x) {
  return(slot(x, "num.nodes"))
})

#' @aliases variables,BN
#' @rdname variables
setMethod("variables", "BN", function(x) {
  return(slot(x, "variables"))
})
...
#' @rdname node.sizes
setMethod("node.sizes", "BN", function(x) {
  return(slot(x, "node.sizes"))
})

#' @aliases cpts,BN
#' @rdname cpts
setMethod("cpts", "BN", function(x) {
  return(slot(x, "cpts"))
})

#' @aliases dag,BN
#' @rdname dag
setMethod("dag", "BN", function(x) {
  return(slot(x, "dag"))
})

#' @aliases wpdag,BN
#' @rdname wpdag
setMethod("wpdag", "BN", function(x) {
  return(slot(x, "wpdag"))
})

#' @aliases scoring.func,BN
#' @rdname scoring.func
setMethod("scoring.func", "BN", function(x) {
  return(slot(x, "scoring.func"))
})

#' @aliases struct.algo,BN
#' @rdname struct.algo
setMethod("struct.algo", "BN", function(x) {
  return(slot(x, "struct.algo"))
})

#' @aliases num.time.steps,BN
#' @rdname num.time.steps
setMethod("num.time.steps", "BN", function(x) {
  return(slot(x, "num.time.steps"))
})
...
setReplaceMethod(
  ...
  function(x, value) {
    if (inherits(value, "character")) {
      slot(x, "discreteness") <- sapply(1:length(value), FUN = function(i) {
        !is.na(match(value[i], c("d", "D")))
      })
    } else { # is logical
...
plot.BN <-
  function(x, method = "default", use.node.names = TRUE, frac = 0.2, max.weight = max(dag(x)),
    ...
    if (method == "default") {
      bngzplot(
        x, mat, num.nodes, variables, use.node.names,
        frac, max.weight, node.size.lab, node.col
      )
    } else {
      plist <- list(...)
      bnqgplot(
        mat, num.nodes, variables, use.node.names,
...
label.edges <- function(dgraph, layering = NULL, layer.struct = NULL) {
  ...
  if (!is.null(layering) && !is.null(layer.struct)) {
    ...
    diag(layers) <- 0
    # This is wrong since it returns a boolean matrix
    # label <- label & layers

    idx.rev.edges <- which(label == -1, arr.ind = TRUE)
    for (i in seq_len(nrow(idx.rev.edges))) {
      row <- idx.rev.edges[i, 1]
      col <- idx.rev.edges[i, 2]
      if (!(layers[row, col] == layers[col, row])) {
        if (layers[row, col]) {
          label[row, col] <- 1
          label[col, row] <- 0
        } else {
